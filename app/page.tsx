...
type SkillIconConfig = { icon: IconType; color?: string };

const skillIcons: Record<string, SkillIconConfig> = {
  'C#': { icon: SiSharp, color: '#512bd4' },
  JavaScript: { icon: SiJavascript, color: '#c89b00' },
  ...
  'SAP B1 Service Layer': { icon: SiSap, color: '#0874be' },
  'Sitecore XP': { icon: SiSitecore, color: '#eb1f1f' },
  ...
  Swagger: { icon: SiSwagger, color: '#75bd36' },
  ...
  Codex: { icon: FaRobot, color: '#10a37f' },
  'Python (working knowledge)': { icon: SiPython, color: '#3776ab' },
  ...
  return <>
    ...
    <main id="main">
      ...
      <section className="section container" id="skills">
        ...
        <div className="skills-board">{Object.entries(skills).map(([category, items], i) => <div className={`skill-cell skill-cell-${i + 1}`} key={category}><div className="skill-top"><span className="skill-symbol" aria-hidden="true">{['{ }', '</>', '↔', '▤', '☁', '✓', '✳', 'λ', '◎', '↗', 'AI', 'ƒ'][i]}</span><h3>{category}</h3><span className="skill-number">{String(i + 1).padStart(2, '0')}</span></div><div className="skill-items">{items.map(skill => { const config = getSkillIcon(skill); const SkillIcon = config.icon; return <span key={skill}><SkillIcon className="skill-item-icon" style={{ color: config.color }} aria-hidden="true" focusable="false"/>{skill}</span>; })}</div></div>)}</div>
      </section>
